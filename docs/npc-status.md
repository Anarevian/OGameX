...
**Last updated:** 2026-08-09 (early-game growth fixes, defects 25–28)
...
| Early game | `./vendor/bin/phpunit --filter BotGrowthTest` | **Pass** — 3 tests, 36 assertions (~74s) |
| Full bot suite | `php artisan test --filter="Bot*Test"` | **Pass** — 67 tests, 643 assertions (~46 min) |
...
| 28 | **A session queued the same building five times.** Every score is derived from `getObjectLevel()`, which reports what is *built* and knows nothing about what is *queued*. The brain re-proposes after each action, so a mine that stayed at its pre-queue level kept its pre-queue payback — and payback is what makes a mine win. One session produced "metal mine 2" five times over, and because mines filled all five queue slots every time, the solar plant, robot factory, research lab and shipyard were never reached at all. The accounts were permanently locked out of research and shipbuilding. | `canBuild()` now rejects a building that already has an entry in the planet's queue, so a session queues at most one of each. That is also what a competent player does early on: a mine, a different mine, a solar plant, a lab — not the same mine five times. |
| 27 | **Fresh bots never developed at all** — the symptom a server owner actually reports, and the sum of defects 25, 26 and this one. Measured on four fresh Miners over a simulated week: metal mine 5, and zero solar plants, robot factories, research labs, shipyards or technologies. After all three fixes the same cohort reached metal 16–18, crystal 13–16, deuterium 12–15, solar 15–19, robot 2–4, lab 1–3, shipyard 1–3 and energy technology 4. Decisions went from 97 (100% buildings) to 474 (80% building / 15% research / 6% defence). | Covered end to end by `BotGrowthTest`, which asserts the outcome rather than the mechanism. Every one of these three ticked happily and failed no test while producing an account that owned nothing. |
| 26 | **The Abstract level of detail was not the cadence-only change it claimed to be.** It stretches the gap between sessions six-fold. The building queue holds five items, so a session cannot make up the difference however large its action budget is — a distant bot simply grew six times slower, permanently. With one human on the server almost every bot is Abstract, so this applied to nearly the whole population. | Two parts. The session action budget is now scaled by the same multiplier, which restores the intent for actions that are not queue-bound. And an account younger than `BOTS_FULL_CADENCE_DAYS` (14) keeps full cadence regardless of detail level: slowing an empire whose next upgrade takes days is a real saving, slowing an account that owns nothing is just a sentence. Age is read from the user's registration date, so `--developed` populations are correctly treated as established from the first turn. |
| 25 | **Research was outbid by mines every single step, and then starved.** Research saturated towards 2.0 while buildings saturated towards 3.0, so a Miner's energy technology scored 2.75 against an early mine's 2.88 — and the mine then spent the crystal the research needed, so it was unaffordable at the next step too. Across a simulated week: 97 building decisions, zero research. Losing narrowly and repeatedly to something that consumes your budget is indistinguishable from never being considered. | Research saturates towards the same 3.0 ceiling. It only ever proposes when the research queue is idle, and an idle research queue is waste no real player tolerates, so competing on equal terms with a mine is the honest scoring rather than a thumb on the scale. Category fatigue then hands the following slots back to buildings. |
